...
boolean contains_destructible(int x, int y){
...
boolean contains_enemy(int x, int y){
...
boolean contains_edible(int x, int y){
...
String closest_direction(int x, int y){
...
void decrease_destructible(int x, int y){
  for (Destructible temp : destructibles){
...
boolean is_in_frame(int x, int y){
...
void animation_pause(Character chr){
...
void level_pause(int time, Game_State state, boolean loop_flag){
  Timer timer = new Timer();
...
void set_floor(){
  for (int i = 0; i < floor.length; i++){
    for (int j = 0; j < floor[0].length; j++){
      PImage temp = loadImage("../images/floor/" + (int(random(1, 9))) + ".png");
      temp.resize(tile_size,tile_size);
      floor[i][j] = temp;
    }
  }
  ...
  for (int i = 0; i < wall.length; i++){
    for (int j = 0; j < wall[0].length; j++){
      if (i == 0 || i == wall.length - 1 || j == 0 || j == wall[0].length - 1){
        PImage temp = loadImage("../images/walls/" + (int(random(1, 5))) + ".png");
        temp.resize(tile_size,tile_size);
        wall[i][j] = temp;    
      }
    }
...
class Character{
  ...
  void setAnimation(String animation, int speed){
